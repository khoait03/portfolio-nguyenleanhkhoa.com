...
namespace App\Filament\Resources\SiteSetting;

use App\Filament\Resources\SiteSetting\SiteSettingResource\Pages;
                        ...
                        Tabs\Tab::make('Mạng xã hội')
                            ->schema([
                                ...
                                TextInput::make('linkedin')
                                    ->label('Linkedin')
                                    ->nullable()
                                    ->afterStateHydrated(fn ($state, callable $set) => $set('linkedin', self::addHttpsIfMissing($state)))
                                    ->dehydrateStateUsing(fn ($state) => self::addHttpsIfMissing($state)),
                            ])->columnSpan('full'),
                    ]),
            ])->columns(1);
